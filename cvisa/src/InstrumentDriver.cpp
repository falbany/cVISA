...
const std::map<std::string, CommandSpec> InstrumentDriver::s_common_command_registry = {
...
bool InstrumentDriver::isOperationComplete() {
    return trim(executeCommand(s_common_command_registry.at("op_complete_q"))) == "1";
}

int InstrumentDriver::runSelfTest() {
    std::string response = trim(executeCommand(s_common_command_registry.at("self_test_q")));
    try {
...
uint8_t InstrumentDriver::getStatusByte() {
    std::string response = trim(executeCommand(s_common_command_registry.at("status_byte_q")));
    try {
...
uint8_t InstrumentDriver::getEventStatusRegister() {
    std::string response = trim(executeCommand(s_common_command_registry.at("event_status_q")));
    try {
        return static_cast<uint8_t>(std::stoi(response));
    } catch (const std::exception& e) {
...
uint8_t InstrumentDriver::getEventStatusEnable() {
    std::string response = trim(executeCommand(s_common_command_registry.at("event_enable_q")));
    try {
        return static_cast<uint8_t>(std::stoi(response));
    } catch (const std::exception& e) {
...
uint8_t InstrumentDriver::getServiceRequestEnable() {
    std::string response = trim(executeCommand(s_common_command_registry.at("service_req_en_q")));
    try {
        return static_cast<uint8_t>(std::stoi(response));
    } catch (const std::exception& e) {
